...
### Zasada
Admin zarządza wszystkimi treściami przez wspólny moduł administracyjny.
To obejmuje:
- wpisy użytkowników,
- anonimowe wiadomości,
- inspiracje.

Takie podejście rozwiązuje rozjazd między wymaganiem "zarządzania inspiracjami" a brakiem osobnego ekranu admin inspiracji w `SCREENS.md`.

| Ścieżka | Metoda | Widok / ekran | Dostęp | Cel |
|---|---|---|---|---|
| `/admin` | GET | `admin/dashboard.ejs` | admin | panel administratora |
| `/admin/entries` | GET | `admin/admin-entries.ejs` | admin | lista wszystkich treści z filtrem typu i statusu |
| `/admin/entries/:kind/:id/edit` | GET | `admin/admin-entry-edit.ejs` | admin | edycja wpisu, wiadomości lub inspiracji |
| `/admin/entries/:kind/:id` | POST | redirect do `/admin/entries` | admin | zapis zmian treści |
| `/admin/entries/:kind/:id/status` | POST | redirect do `/admin/entries` | admin | zmiana statusu treści |
| `/admin/entries/:kind/:id/delete` | POST | redirect do `/admin/entries` | admin | usunięcie lub soft delete |
| `/admin/help/edit` | GET | `admin/admin-help-edit.ejs` | admin | edycja sekcji pomocy |
| `/admin/help/:id` | POST | redirect do `/admin/help/edit` | admin | zapis zmian sekcji pomocy |

Parametr `:kind` przyjmuje wyłącznie:
- `entry`,
- `message`,
- `inspiration`.
...
- osobny moduł odzyskiwania hasła.

## Ryzyka i miejsca łatwe do pominięcia
- `POST /logout` zastąpiony przez `GET`.
- Brak sprawdzenia właściciela przy `/entries/:id/edit`.
- Niespójne nazwy między widokami a folderami Stitch.
- Pominięcie filtrów typu treści w `/admin/entries`, przez co admin nie będzie realnie zarządzał inspiracjami.
