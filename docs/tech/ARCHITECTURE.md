...
Nie opisuje pełnego stanu implementacji.
Aktualny stan repo opisuje `docs/tech/IMPLEMENTATION_STATUS.md`.

## Kontekst MVP
Hierarchia źródeł prawdy jest utrzymywana w `docs/README.md`.
Ten dokument korzysta z niej, ale jej nie duplikuje.
...
Ta struktura jest już w repo utworzona jako skeleton.
Najważniejsze braki nie dotyczą układu katalogów, tylko zawartości:
- logiki w `services/` i `repositories/`,
- finalnych migracji i seedów,
- realnego CSRF,
- finalnych widoków EJS.
...
## Dodatkowe katalogi root

### `scripts/db/`
To pomocnicza warstwa operacyjna dla:
- migracji,
- seedów,
- resetu danych developerskich.

### `docker/`
To miejsce na lekkie pliki pomocnicze kontenerów.
Obecnie zawiera `postgres-init/` z inicjalizacją PostgreSQL.

### Root repo
Pliki root są częścią architektury developerskiej:
- `Dockerfile`
- `docker-compose.yml`
- `.env.example`
- `package.json`
...
- Traktować `src/public/css/tailwind.input.css` jako wejście builda.
...
- Łatwo pomylić strukturę repo ze stanem ukończenia modułów. Dlatego zawsze trzeba sprawdzać `docs/tech/IMPLEMENTATION_STATUS.md`.
